 ...
 <section>
     ...
     <div> 
            <div id="search" >
             ...
             <div class="block"  style="float: left;margin-top: 10px;margin-left:10px">
                <el-select v-model="value" placeholder="选择身份类别"  @change="filterRate"  clearable >
                    <el-option
  ...
  <el-table  :data="results" highlight-current-row  v-loading="listLoading" @selection-change="selsChange" style="width: 100%" label-width="0px" >
    ...
    <el-table-column label="操作" width="150">
          <template scope="scope">
            <el-button v-if="scope.row.Isseller" type="success" size="small" @click="viewCompanyInfo(scope.row)">查看公司信息</el-button>
        </template>
      ...
      <el-pagination layout="prev, pager, next" @current-change="handleCurrentChange" :page-size="20" :total="total" style="float:right;">
       ...
       <div>
            ...
            <el-table :data="showResults"  style="width: 100%; margin-bottom:10px">
                <el-table-column  fixed  prop="companyName" label="公司名称"  width="300">
                </el-table-column>
                <el-table-column  prop="companyRegisterId" label="营业执照注册号"  width="220">
                </el-table-column>
                <el-table-column prop="companyInstitutionalId"  label="组织机构代码"  width="190">
                </el-table-column>
                <el-table-column prop="comanyAddress"  label="常用地址"  width="120">
                </el-table-column>
                <el-table-column prop="companyTellNumber"  label="联系电话"  width="150">
                </el-table-column>
                <el-table-column prop="CompanyLicenseAddress"  label="营业执照所在地"  width="150">
                </el-table-column>
                <el-table-column prop="RegisterMoneny"  label="注册资金"  width="120">
                </el-table-column>
                <el-table-column prop="LegalPersonName"  label="法定法人姓名"  width="120">
                </el-table-column>
                <el-table-column prop="LegalPersonIDCard"  label="身份证号"  width="200">
                </el-table-column>
                <el-table-column prop="companyInstitutionalId"  label="公司账号"  width="170">
                  <template scope="scope">
...
<script > 
  ...
  methods: {
    ...
    selectGoodsInfo() {  //显示所有的用户信息
      // var name = this.userName;
      this.listLoading = true;
      this.$http.post('./api/user/selectPersonInfo', {
          page: this.page,
        ...
        for (var index = 0; index < this.results.length; index++) {
          ...
          if(element.user_type == 'seller'){
              element.Isseller = true;
          }
        }
    ...
    StatusOperation(UserType){
      ...
      filterRate(){ //筛选状态
          ...
          if (this.results.length != 0 && this.value!='') {
            switch (val) {
              case '1':
                  this.StatusOperation('root'); //管理员
                  break;
              case '2':
                  this.StatusOperation('buyer');//买家
                  break;
              case '3':
                  this.StatusOperation('seller'); //卖家
                  break;
              }
          }else{
                  this.selectGoodsInfo();
    ...
    viewCompanyInfo(row){
          ...
          var companyRegisterId = row.companyRegisterId;
          this.selectCompanyInfo(companyRegisterId);
          
    },

     selectCompanyInfo(companyRegisterId) {  //显示公司信息
      // var name = this.userName;
      this.$http.post('./api/user/selectCompanyInfo', {
         companyRegisterId: companyRegisterId
      },{}).then((response) => {
        //  console.log(response.body[0]); 
         
        this.showResults.length = 0;
        this.showResults.push(response.body[0]);
